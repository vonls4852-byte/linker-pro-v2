...
import { GroupInfoModalProps } from './types';
...
export default function GroupInfoModal({
  ...
  // ==================== 2. ЗАГРУЗКА ДАННЫХ ====================
  useEffect(() => {
    if (chat?.participants) loadParticipants();
  }, [chat]);
  ...
  const loadParticipants = async () => {
    ...
    try {
      ...
      for (const userId of chat.participants) {
        ...
        if (result.user) data.push(result.user);
      }
  ...
  const handleToggleAdmin = async (userId: string, action: 'add' | 'remove') => {
    ...
    try {
      ...
      if (response.ok) {
        if (onToggleAdmin) onToggleAdmin(userId, action);
  ...
  const handleAddClick = () => {
    if (onAddParticipants) onAddParticipants();
  };

  const handleLeaveClick = () => setConfirmAction('leave');
  const handleDeleteClick = () => setConfirmAction('delete');

  const handleConfirm = () => {
    if (confirmAction === 'leave' && onLeaveGroup) onLeaveGroup();
    if (confirmAction === 'delete' && onDeleteGroup) onDeleteGroup();
      ...
      <div className="bg-[#111] rounded-2xl p-6 w-full max-w-md border border-white/10">
        ...
        <div className="space-y-2">
          {isAdmin && (
            <button
              onClick={handleAddClick}
              className="w-full py-3 bg-blue-500/20 hover:bg-blue-500/30 rounded-xl text-blue-400 font-medium flex items-center justify-center gap-2"
            >
              <UserPlus size={18} /> Добавить участников
            </button>
          )}
          <button
            onClick={handleLeaveClick}
            className="w-full py-3 bg-yellow-500/20 hover:bg-yellow-500/30 rounded-xl text-yellow-400 font-medium flex items-center justify-center gap-2"
          >
            <LogOut size={18} />{' '}
          ...
          {isCreator && (
            <button
              ...
              className="w-full py-3 bg-red-500/20 hover:bg-red-500/30 rounded-xl text-red-400 font-medium flex items-center justify-center gap-2"
            >
...
function ParticipantItem({
  ...
  const canManage = isCreator && !isUserCreator;
    ...
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[70] flex items-center justify-center p-4">
      <div className="bg-[#111] rounded-2xl p-6 w-full max-w-sm border border-white/10">
        ...
        <div className="flex gap-3">
          <button
            onClick={onConfirm}
            className={`flex-1 py-2 bg-${msg.color}-500/20 hover:bg-${msg.color}-500/30 rounded-lg text-${msg.color}-400 font-medium`}
          >
            {msg.confirm}
          </button>
          <button
            onClick={onCancel}
            className="flex-1 py-2 bg-white/5 hover:bg-white/10 rounded-lg text-white font-medium"
          >
            Отмена
          </button>
        </div>
